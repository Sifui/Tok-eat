...
exports.generateAddress = function generateAddress () {
    ...
    finalAdress = String(parseInt(Math.random()* 10)) + " " + addresses[parseInt(Math.random()*(addresses.length - 0))]
    
    return finalAdress
}

exports.generateRestaurantNameAndDescription = function generateRestaurantNameAndDescription(){
    ...
    const descriptions = `HISTORIQUE - Fondée en 1962 dans le quartier du Marché de la Gare, la Brasserie Midi Minuit a traversé le temps… Plus de 4 millions de clients plus tard et une réputation qui n'est plus à faire : bienvenue dans le nouvel espace du resto, entièrement rénové. 
    ...
    const lineNames = names.split('\n')
    const name = Array.from(new Set(lineNames.map(l => l.trim())))
    
    const lineDescriptions = descriptions.split('\n')
    const description = Array.from(new Set(lineDescriptions.map(l => l.trim())))

    if(name.length === description.length)
    {
        let NameAndDescription = []
        name.forEach((nameOffer,index) => {
            NameAndDescription.push({name:nameOffer, description:description[index]})
        });
        return NameAndDescription
    }
    else
    {
        console.log("error length of description or name in server/utils/dataFunction/offerDataFunction.js ")
        return false
    }
}
